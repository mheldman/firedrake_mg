# python test.py -d zznonflatrough.py -o ./testresults/nonflatrough/ --mgtype pfas --numlevels 5 --rtol 1e-12 --maxiters 50 --atol 1e-20 --innerits 1 --fmg true --fmgc 2 --cycle V --preiters 2 --postiters 2 --eps 0.0

# python test.py -d zznonflatrough.py -o ./testresults/nonflattilt/ --mgtype gmg --maxiters 50 --atol 1e-20 --rtol 1e-12 --numlevels 6 --preiters 2 --postiters 2 --eps 0.0 --fmg true --fmgc 3
...
gc = 9.81 #m / s^2
A = 10**(-16) #Pa^-3 a^-1
a = 100. #parameter controlling the periodicity of the bed and hence the roughness. a = 750 is fairly smooth

def b(x, y):
  r = sqrt((x - L)**2 + (y - L)**2)
  return conditional(r > .1, -b0*cos(z0*pi*r/a), -b0)

cmesh = RectangleMesh(100, 100, 2*L, 2*L, quadrilateral=True)
...
def f(x, y):
  r = sqrt((x - L)**2 + (y - L)**2)
  C = 1.28e22/19208566844536797.#-1.0#
  return   conditional(r < R - 0.1, conditional(r > .1,  -((3*3**0.041666666666666664*
             ((5*2**0.5416666666666666*3**0.2916666666666667*
               ...
               (72*sqrt(5)*pi**2*
                  ...
                  cos((6*pi*r)/(5.*a)))/a**2 -
               (50*sqrt(5)*pi*(-30 +
                    6**0.6666666666666666*(750 - r)**0.3333333333333333 +
                    6**0.6666666666666666*r**0.3333333333333333)*
                  sin((6*pi*r)/(5.*a)))/
                (a*(-7500 + 6**0.6666666666666666*
                      (750 - r)**1.3333333333333333 + 40*r -
                     6**0.6666666666666666*r**1.3333333333333333)**0.375))*
             ((3**0.6666666666666666*
                   (-30 + 6**0.6666666666666666*(750 - r)**0.3333333333333333 +
                     6**0.6666666666666666*r**0.3333333333333333))/1250. -
                (24*3**0.6666666666666666*pi*
                   (1 - (3*(1 - (1 - r/750.)**1.3333333333333333 -
                           (2*r)/1125. +
                           r**1.3333333333333333/(3750.*6**0.3333333333333333)))
                        /2.)**0.625*sin((6*pi*r)/(5.*a)))/(5.*a))**2)/
           (3125.*2**0.875) + (-(3**0.6666666666666666*
                  (-30 + 6**0.6666666666666666*(750 - r)**0.3333333333333333 +
                    6**0.6666666666666666*r**0.3333333333333333))/1250. +
              (24*3**0.6666666666666666*pi*
                 (1 - (3*(1 - (1 - r/750.)**1.3333333333333333 - (2*r)/1125. +
                          r**1.3333333333333333/(3750.*6**0.3333333333333333)))/
                    2.)**0.625*sin((6*pi*r)/(5.*a)))/(5.*a))**3/r), C*1555893914407480557/312500000000000000000000000000), -C*1555893914407480557/6250000000000000000000000000000)


  '''
  conditional(r < R - 0.1, conditional(r > .1,  -(3/8)**3*(((3*3**0.041666666666666664*
  ...
  '''
  '''
  conditional(r < R - 0.1, conditional(r > .1,  -((3*3**0.041666666666666664*
             ((5*2**0.5416666666666666*3**0.2916666666666667*
                  (-(750 - r)**0.6666666666666666 + r**0.6666666666666666))/
                (-((-750 + r)*r))**0.6666666666666666 +
               (72*sqrt(5)*pi**2*
...
def form(u, v):
   ...
   C = 2.*A*(rho*gc)**(p-1)*1.28e22/19208566844536797.#*1.28e22/19208566844536797.
   return C*u**q/(p+1)*(inner(grad(u) + grad(b(x, y)), grad(u) + grad(b(x, y))))*inner(grad(u) + grad(b(x, y)), grad(v))*dx #+ u*v*dx
...
eps = 1e-2
def jac(u, w, v):
    #w is the picard variable, u is the newton variable.
    # if the coefficient u**q is replaced with w**q, each newton step is for a
    # degenerate p-Laplace problem with tilt. if regularized (eps != 0), it's
    # nondegenerate and the PGS solver should converge for the linearized problem
    x, y = SpatialCoordinate(u)
    C = 2.*A*(rho*gc)**(p-1)*1.28e22/19208566844536797.#*1.28e22/19208566844536797.
    a =  C*(w + eps)**q/(p+1)*(inner(grad(u) + grad(b(x, y)), grad(u) + grad(b(x, y))))*inner(grad(u) + grad(b(x, y)), grad(v))*dx
    return derivative(a, u)
'''
