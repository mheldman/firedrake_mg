...
def build_levels(numlevels):
  ...
  for mesh in mh:
    ...
    u = Function(V)
    a    = inner(grad(u), grad(v))*(1./sqrt(1. + inner(grad(u), grad(u))) + eps)*dx #
    A    = None
    H = TrialFunction(V)
...
if mg_type == 'gmg':
  ...
  if fmg:
    u = gmgsolver.fmgsolve(psi, cycle=cycle, rtol=rtol, atol=atol, maxiters=maxiters, innerits=1, j=3)
  else:
  ...
  if fmg:
    u = gmgsolver.fmgsolve(f, psi, cycle=cycle, rtol=rtol, atol=atol, maxiters=maxiters, j=3)
  else:
    f = assemble(f*v*dx, bcs=levels[0].bcs)
...
if numlevels < 8:
  ...
  plot(u, plot3d=True)
  plt.savefig('test results/ncp/mseradialsquare/' + mg_type + 'radialu')

plot(u)
plt.savefig('test results/ncp/mseradialsquare/' + mg_type + 'radialucolor')
...
e.assign(u - uexact)
plot(e)
plt.savefig('test results/ncp/mseradialsquare/' + mg_type + 'error')
