...
def build_levels(numlevels):
  ...
  for mesh in mh:
    ...
    u = Function(V)
    a    = (u**2*inner(grad(u), grad(u))*inner(grad(u), grad(v)) + eps*inner(grad(u), grad(v)) + delt*inner(grad(u), grad(u))*inner(grad(u), grad(v)))*dx
    A    = None
    H = TrialFunction(V)
...
class psi(Expression):
  def eval(self, value, x):
    value[:] = -1000.0#psih*sin(pi*x[0]/4.)*sin(pi*x[1]/4.)
class cmb(Expression):
  def eval(self, value, x):
...
plot(u)
plt.savefig('test results/ncp/icedome/u')
