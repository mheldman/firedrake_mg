#python test.py -d zzpbratu.py -o ./testresults/lcp/bratu/ --mgtype pfas --maxiters 50 --atol 1e-14 --rtol 1e-12 --eps 0.0 --numlevels 10 --preiters 2 --postiters 2 --fmgc 2 --cycle V --fmg true
...
coarsemx = 5 #these have to specified in the user file along with element types, which is annoying
coarsemy = 5
...
def form(u, v):
  return inner(grad(u), grad(u))**((p-2)/2)*inner(grad(u), grad(v))*dx + 2.5*exp(u)*v*dx
