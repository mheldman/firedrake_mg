...
def build_levels(numlevels):
  ...
  for mesh in mh:
    ...
    a    = ((1. - eps)*Gamma*(H**5 + rpl**2) + eps*D0)*inner(grad(H), grad(v))*dx
...
'''
t0 = (18.*Gamma)**(-1)*(7./4.)**3*domeR**4*domeH0**(-7)
...
'''
u = Function(V)
u.interpolate(10.*(y/(2*L))**4*(1. - (x/(2.*L))**4.)*(1. - (y/(2.*L))**4)*(x/(2*L))**4)
...
while eps <= 1.0:
  ...
  plot(assemble(inner(grad(u), grad(u))*v*dx))
  plt.savefig(mgtype + 'gradicesol.png')
