...
def build_levels(numlevels):
  ...
  for mesh in mh:
    ...
    lvl = licplevel(mesh, V, a, b, A, B, bcs, u, findices=None, M=M, bindices=bndry)
...
plot(levels[0].mesh)
plt.savefig('test results/lcp/radialcircle/finemesh')
plot(levels[-1].mesh)
plt.savefig('test results/lcp/radialcircle/coarsemesh')
...
if numlevels < 5:
  u.assign(u)
  plot(u, plot3d=True)
  plt.savefig('test results/lcp/radialcircle/' + mg_type + 'radialu')
