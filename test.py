...
def main(argv):
  ...
  if probtype == "iceflat":
    def form(u, v):
      return u**q*inner(grad(u), grad(u))**((p-2)/2)*inner(grad(u), grad(v))*dx
  elif probtype == "plaplace":
    def form(u, v):
      return inner(grad(u), grad(u))**((p-2)/2)*inner(grad(u), grad(v))*dx
  elif probtype == "mse":
    def form(u, v):
      return (1./sqrt(1. + inner(u, u)))*inner(grad(u), grad(v))*dx
  ...
  if ploterror3d:
    print("plotting 3d error..")
    plot(u - uexact, plot3d=True)
    plt.savefig(output + "3derror.png")
  if resheatmap:
